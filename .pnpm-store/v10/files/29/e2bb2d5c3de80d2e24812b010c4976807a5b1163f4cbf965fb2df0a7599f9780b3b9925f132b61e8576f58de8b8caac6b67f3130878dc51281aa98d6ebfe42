...
var _flat = require("../../lib/utils/flat");
...
function getAttributesToClear(param) {
    ...
    return (0, _utils.uniq)((0, _utils.getRefinements)(results, helper.state, true).map(function(refinement) {
...
function updateStateFromSearchToolInput(params, helper) {
    ...
    if (params.facetFilters) {
        var attributes = (0, _flat.flat)(params.facetFilters).map(function(filter) {
...
var _default = function connectChat(renderFn) {
    var unmountFn = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : _utils.noop;
    (0, _utils.checkRendering)(renderFn, withUsage());
    return function(widgetParams) {
        (0, _utils.warning)(false, 'Chat is not yet stable and will change in the future.');
        ...
        var makeChatInstance = function makeChatInstance(instantSearchInstance) {
            ...
            if ('transport' in options && options.transport) {
                ...
                transport = new _ailite.DefaultChatTransport(_object_spread_props._(_object_spread._({}, options.transport), {
            ...
            return new _chat.Chat(_object_spread_props._(_object_spread._({}, options), {
                transport: transport,
                ...
                onToolCall: function onToolCall(param) {
                    ...
                    if (!tool && toolCall.toolName.startsWith("".concat(_chat.SearchIndexToolType, "_"))) {
                        tool = tools[_chat.SearchIndexToolType];
                    }
                    if (!tool) {
        ...
        return {
            ...
            init: function init(initOptions) {
                ...
                setFeedbackState = function setFeedbackState(messageId, state) {
                    feedbackState = _object_spread_props._(_object_spread._({}, feedbackState), _define_property._({}, messageId, state));
                    render();
                };
                var feedback = 'feedback' in options ? options.feedback : undefined;
                if (agentId && feedback) {
                    var _getAppIdAndApiKey = _sliced_to_array._((0, _utils.getAppIdAndApiKey)(initOptions.instantSearchInstance.client), 2), appId = _getAppIdAndApiKey[0], apiKey = _getAppIdAndApiKey[1];
                    if (!appId || !apiKey) {
                        throw new Error(withUsage('Could not extract Algolia credentials from the search client.'));
                    }
                    ...
                    _sendChatMessageFeedback = function _sendChatMessageFeedback(messageId, vote) {
                        ...
                        (0, _utils.sendChatMessageFeedback)({
                            agentId: agentId,
                            vote: vote,
                            messageId: messageId,
                            appId: appId,
